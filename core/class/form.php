...
class form
{
	...
	private function getFormFields()
	{
		...
		foreach ($this->fields as $name => $props)
		{
			...
			if (isset($props['info']) && !empty($props['info']))
				$form_fields .= '<span class="form-info">' . $props['info'] . '</span>' . "\n";
		}
	...
	private function getFormField($fieldname, array $props)
	{
		...
		$class  = $props['class'] . (isset($props['validInput']) && $props['validInput'] == false ? ' wrong is-invalid' : '');
	...
	private function getFormTemplateURL($formname)
	{
		...
		if (file_exists(MAIN_PATH . '/template/form/' . $formname))
			return MAIN_PATH . '/template/form/' . $formname;
